...
export default function SignUpScreen3(props) {
    ...
    (async function () {
      if (!user.tz) {
        await firestore
          .collection('users')
          .doc(user.uid)
          .set({ tz: moment.tz.guess() }, { merge: true });
      }
    })();
  }, []);
  ...
  async function onFinish(days) {
    await Promise.all(
      Object.entries(days).map(async ([key, value]) => {
        if (value.range) {
          await firestore
            .collection('users')
            .doc(user.uid)
            ...
            .set(
              {
                day: key,
                end: value.range[1].format('YYYY-MM-DDTHH:mm:00'),
                start: value.range[0].format('YYYY-MM-DDTHH:mm:00'),
              },
              { merge: true },
            );
        }
  ...
  if (user === null) {
    return <div>you need to be signed in to access this resource</div>;
  }
      ...
      <Row justify="center">
        <Col span={12}>
          <Row justify="center">
            ...
            <Title
              level={5}
            >{`We'll use these times to create your appointment slots. You can make changes later.`}</Title>
          </Row>
        </Col>
      </Row>
      <Row justify="center">
